...
class SearchService:
    ...
    async def search_serpapi(query: str) -> List[SearchResult]:
        """Search using ZenSERP API"""
        api_key = os.getenv("ZENSERP_API_KEY")
        
        if not api_key:
            print("[search] ZENSERP_API_KEY not set; returning empty results")
            return []
    ...
    async def search_brave(query: str) -> List[SearchResult]:
        ...
        if not api_key:
            print("[search] BRAVE_SEARCH_API_KEY not set; returning empty results")
            return []
        ...
        try:
            ...
            results = []
            for i, result in enumerate(data.get("web", [])[:SearchService.MAX_RESULTS_BRAVE]):
                results.append(
                    SearchResult(
                        position=i + 1,
